...
void content_shuffle_hand(card* head) // Same as regular content_shuffle, minor changes to allow for noncontiguous memory blocks
{
	// This function works by keeping the list structure itself unchanged, swapping the contents of cards to shuffle the deck
	...
	for (int i = 0; i < size; ++i)
	{
		...
		for (int j = 0; j < i; ++j)
		{
			current = current->next; // Ptr to first card to be swapped
		}

		target_ind = rand() % size;
		target = head;
		for (int j = 0; j < target_ind; ++j)
		{
			target = target->next; // Ptr to second card to be swapped
		}
...
void node_remove_special(card* head, card* target)
{
	...
	int type;
	if (head->face == target->face && head->suit == target->suit) type = 1;
	else if (target->next == NULL) type = 3;
	else type = 2;
	...
	case 1:
		...
		*head = *head->next;
		...
		return;
	case 2:
		for (current = head; current->next->face != target->face && current->next->suit != target->suit; current = current->next); // Iterate to card before target
